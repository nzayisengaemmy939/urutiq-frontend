...
export const useRealtimeAccounting = ({ tenantId, companyId }) => {
    const { setAccounts, setAccountsError } = useAccountingStore();
    const handleAccountingUpdate = useCallback((event) => {
        try {
...
export const useOptimisticUpdates = () => {
    ...
    const optimisticUpdateAccount = useCallback(async (accountId, updates, apiCall) => {
        // Apply optimistic update
        const optimisticAccounts = accounts.map((acc) => acc.id === accountId ? { ...acc, ...updates } : acc);
        setAccounts(optimisticAccounts);
        try {
            // Make API call
            const updatedAccount = await apiCall();
            // Apply real update
            const realAccounts = accounts.map((acc) => acc.id === accountId ? updatedAccount : acc);
