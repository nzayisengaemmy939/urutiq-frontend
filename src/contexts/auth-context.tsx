...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const ensureCompanyId = async (payload?: JwtPayload) => {
    try {
      ...
      const companies = await apiService.getCompanies({ page: 1, pageSize: 1 }) as any
      const list = (companies?.data as any[]) || (companies as any[]) || []
      ...
      if (first?.id) {
        ...
        if (!existing || list.length === 1 || existing !== first.id || isDemo) {
          ...
          if (first.name) localStorage.setItem("company_name", first.name)
        }
      }
    } catch (error) {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        const tokens = await apiService.verifyMfaLogin((result as { ok: false; challengeToken: string }).challengeToken, code)
        const payload = parseJwt<JwtPayload>(tokens.accessToken)
        if (!payload) throw new Error('Invalid JWT received')
        const tenantId = payload.tenantId || "tenant_default"
        try { localStorage.setItem("auth_token", tokens.accessToken) } catch {}
        try { if (tokens.refreshToken) localStorage.setItem("refresh_token", tokens.refreshToken) } catch {}
        try { localStorage.setItem("tenant_id", tenantId) } catch {}
        apiService.setAuth(tokens.accessToken, tenantId)
        if (tokens.refreshToken) apiService.setRefreshToken(tokens.refreshToken)
        await ensureCompanyId(payload)
        setUser({
          id: payload.sub,
          email,
      ...
      const accessToken = (loginResp as any).accessToken ?? (loginResp as any).access_token ?? (loginResp as any).token ?? (loginResp as any).data?.accessToken
      const refreshToken = (loginResp as any).refreshToken ?? (loginResp as any).refresh_token ?? (loginResp as any).data?.refreshToken ?? (loginResp as any).refreshToken
  ...
  const logout = async () => {
    ...
    try {
      const rt = localStorage.getItem("refresh_token")
      if (rt) await apiService.logout(rt)
    } catch (error) {
