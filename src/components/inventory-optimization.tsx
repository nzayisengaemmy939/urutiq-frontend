...
export function InventoryOptimization({ 
  ...
  const stockAccuracy = useMemo(() => {
    if (movements.length === 0) return 100
  ...
  const recommendations = useMemo((): OptimizationRecommendation[] => {
    ...
    products.forEach(product => {
      ...
      const outgoingMovements = recentMovements.filter(m => 
        ['OUTBOUND', 'SALE', 'TRANSFER_OUT', 'ADJUSTMENT_OUT', 'RETURN_OUT', 'DAMAGE', 'THEFT'].includes(m.movementType)
      ).length
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      {products.filter(p => {
                        ...
                        const recentMovements = productMovements.filter(m => 
                          new Date(m.movementDate) >= last30Days
                        )
                        const outgoingMovements = recentMovements.filter(m => 
                          ['OUTBOUND', 'SALE', 'TRANSFER_OUT', 'ADJUSTMENT_OUT', 'RETURN_OUT', 'DAMAGE', 'THEFT'].includes(m.movementType)
                        ).length
                        return outgoingMovements >= 3 // 3+ movements in last 30 days
                      }).length} products showing excellent movement
                    </p>
                  </div>
                </div>
