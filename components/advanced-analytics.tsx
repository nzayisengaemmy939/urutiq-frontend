...
export function AdvancedAnalytics() {
  ...
  const loadAnalyticsData = useCallback(async () => {
    ...
    try {
      // Load insights
      const insightsResponse = await bankingApi.get(`/analytics/insights?industry=${selectedIndustry}`)
      setInsights(insightsResponse.insights || [])

      // Load benchmarks
      const benchmarksResponse = await bankingApi.get(`/analytics/benchmarks/${selectedIndustry}`)
      setBenchmarks(benchmarksResponse.benchmarks || [])

      // Load forecast
      const forecastResponse = await bankingApi.get('/analytics/cash-flow-forecast?months=6')
