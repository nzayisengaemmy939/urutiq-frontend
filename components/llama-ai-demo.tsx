...
export const LlamaAIDemo: React.FC<{ companyId: string }> = ({ companyId }) => {
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || ''
  const [activeTab, setActiveTab] = useState<'conversational' | 'documents' | 'analytics' | 'compliance'>('conversational');
  ...
  const handleConversationalQuery = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setLatencyMs(result.data?.latencyMs ?? null);
        setErrorMessage(null);
      } else {
        setErrorMessage(result.error || 'Failed to process conversational request');
        setResponse(null);
        setLatencyMs(null);
      }
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'Unknown error sending message');
      setResponse(null);
      setLatencyMs(null);
    } finally {
      setIsLoading(false);
  ...
  const handleDocumentQuery = async () => {
    if (!documentContent.trim() || !documentQuery.trim()) return;

    setIsLoading(true);
    setErrorMessage(null);
    try {
      const response = await fetch(`${API_BASE}/api/llama-ai/documents/query`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-tenant-id': localStorage.getItem('tenant_id') || 'tenant_demo',
      ...
      if (result.success) {
        ...
        setErrorMessage(result.error || 'Failed to process document query');
        setResponse(null);
      }
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'Unknown error querying document');
      setResponse(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleComplianceAnalysis = async () => {
    ...
    setErrorMessage(null);
    try {
      const response = await fetch(`${API_BASE}/api/llama-ai/compliance/analyze`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-tenant-id': localStorage.getItem('tenant_id') || 'tenant_demo',
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Unknown error running compliance analysis');
      setComplianceResult(null);
    } finally {
      setIsLoading(false);
    }
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {predictiveResult.forecasts.slice(0, 4).map((forecast, index) => (
                      <div key={index} className="p-3 bg-gray-50 rounded-lg">
                        <div className="flex justify-between items-center mb-1">
                          <span className="font-medium text-gray-900">{forecast.type}</span>
                        ...
                        <div className="text-sm text-gray-500">
                          {forecast.period} • {Math.round(forecast.confidence * 100)}% confidence
                        </div>
                      </div>
                    ))}
                  </div>
